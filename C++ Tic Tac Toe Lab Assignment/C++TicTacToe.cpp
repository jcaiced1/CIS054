...
void displayTicTacToe(char board[3][3])
{
    ...
    for (int row = 0; row < 3; row++) // iterate over rows
    {
        
        cout << "\t";
        for (int col = 0; col < 3; col++) // iterate over columns
        {
            cout << "  " << board[row][col] << "  ";
            if (col != 2) // print vertical separator except for last column
            cout << "|";
        }
        cout << endl;
        
        if (row != 2) // print horizontal separator except for last row
